  ...
  dataset: router({
    getStatistics: publicProcedure.query(async () => {
      try {
        ...
        // Ler status de retreinamento
        const statusFile = join(datasetDir, 'retrain_status.json');
        let retrainingStatus = 'idle';
        let lastRetrain = null;
        
        if (existsSync(statusFile)) {
          try {
            const statusData = JSON.parse(readFileSync(statusFile, 'utf-8'));
            retrainingStatus = statusData.status || 'idle';
            lastRetrain = statusData.last_retrain || null;
          } catch (error) {
            console.error('[DATASET] Erro ao ler status de retreino:', error);
          }
        }
    ...
    getHistory: publicProcedure.query(async () => {
      try {
        const historyFile = '/home/ubuntu/skin_cancer_classifier_k230_page/dataset_incremental/retrain_history.json';
        ...
        // Retornar últimos 10 retreinos, ordenados do mais recente para o mais antigo
        return historyData.slice(-10).reverse();
        
      } catch (error: any) {
        console.error('[DATASET] Erro ao obter histórico:', error);
